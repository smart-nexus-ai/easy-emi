...
export default function ProviderDetailView() {
  ...
  const handleRemoveTermsSet = (index: number) => {
    setTermsSets(termsSets.filter((_, idx) => idx !== index));
  };
          ...
          <div className={`p-[1px] rounded-xl ${CARD_THEMES[0].borderGradient} ${CARD_THEMES[0].glowClass} transition-all`}>
            <div className="bg-white dark:bg-slate-900 rounded-[11px] p-5 flex flex-col gap-4 overflow-hidden">
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Partner name *</label>
                <input
                  type="text"
                  value={providerName}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Short internal description (Optional)</label>
                <input
                  type="text"
                  value={providerDesc}
                  onChange={(e) => setProviderDesc(e.target.value)}
                  placeholder="e.g. Used for high-value mobile finance customers"
                  className="px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-955 focus:outline-hidden focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:focus:ring-2 text-slate-900 dark:text-slate-100"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3.5">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    placeholder="300"
                    className="px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-955 focus:outline-hidden focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:focus:ring-2 text-slate-900 dark:text-slate-100 font-mono-nums"
                  />
                </div>
              </div>
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="bg-white dark:bg-slate-900 rounded-[11px] overflow-hidden flex flex-col h-full">
                        {/* Header of single Set */}
                        <div className="bg-slate-50 dark:bg-slate-950 px-4 py-3 border-b border-slate-150 dark:border-slate-800 flex items-center justify-between">
                          <div className="flex items-center gap-1.5 flex-1 mr-2">
                            ...
                            <input
                              ...
                              className="text-xs font-bold text-slate-800 dark:text-slate-200 bg-transparent focus:outline-hidden border-b border-transparent focus:border-slate-400 w-full"
                            />
                          </div>
                          <button
                        ...
                        <div className="p-4 flex flex-col gap-4">
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Set Intro / Description</label>
                            <textarea
                              ...
                              rows={2}
                              className="px-3 py-1.5 text-xs rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-955 focus:outline-hidden focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:focus:ring-2 text-slate-900 dark:text-slate-100 resize-none"
                            />
                          </div>
                          ...
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Rules List ({ts.rules.length}/5)</label>
                              {ts.rules.length < 5 && (
                                <button
                              ...
                              {ts.rules.map((rule, ruleIdx) => (
                                <div key={ruleIdx} className="flex items-center gap-1.5">
                                  ...
                                  <input
                                    ...
                                    className="px-3 py-1 text-xs rounded-md border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-955 focus:outline-hidden focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:focus:ring-2 text-slate-900 dark:text-slate-100 flex-1"
                                  />
                                  <button
